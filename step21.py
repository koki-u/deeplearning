...
####################################
#a * b　やa + bなど簡単に書くことができるようになってきた。
#まだ、　a * np.array(2.0)　のような計算はできない。
#数値データと合わせて計算できるようになりたいってのがある。
#やることは、ndarrayインスタンスをVariableインスタンスに変換するということ。

#普通の数字とnp.arrayを混合して使えるようになった。
x = Variable(np.array(2.0))
y = x + np.array(3.0)
print(y)
...
#問題点として、左の項がfloatやintの場合は？
y = 2.0 * x
#無理ね。

#__rmul__メソッドは自分で記述しないといけないという謎な展開。どういうことよ。笑
