...
class Layer:
    ...
    #dictにパラメータの名前を保存する。
    #paramsメソッドでは_pramsからnameを取り出し、そのnameに対応するオブジェクトをobjとして取り出す。
    #objがlayerのインスタンスの場合にはobj.params()を呼び出す。
    def params(self):
...
class Linear(Layer):
    #入力サイズと出力サイズとバイアスを使用するかどうかをうけとる
...
#setattrはインスタンス変数を設定する際に呼び出される特殊メソッド.
#name はインスタンス変数の名前、valueはインスタンス変数の値を引数に渡す。
#このメソッドがオーバーライドしてくれるからインスタンス変数を追加するときにオリジナルの処理ができる。
    def __setattr__(self, name, value):
        #isinstanceにLayer追加する。これにより、さらにLayerの中にあるLayerを扱えるようにする。
        if isinstance(value, (Parameter, Layer)):
            self._params.add(name)
        super().__setattr__(name, value)
    ...
    #__call__が入力を受け取り、forwardメソッドをよびだす。
    #forwardメソッドは継承先の方で実装する。
    def __call__(self, *inputs):
        outputs = self.forward(*inputs)
        if not isinstance(outputs, tuple):
            outputs = (outputs,)
    ...
    #_paramsにParameterのインスタンス変数名or Layerのインスタンス変数名が保持されている
    #obf = self.__dict__[name]として取り出す必要がある
    #objがLayerの場合には、objの_flatten_paramsメソッドを呼び出すことができる
    #これを再帰的な呼び出しにより実現している。
    ...
    #まず、self.to_cpu()で、データがメインメモリ上に存在することを保証する
    #次ディクショナリ作る
    #データを外部ファイルに保存
    def save_weights(self, path):
        ...
        try:
            ...
            if os.path.exists(path):
                os.remove(path)
            raise
    #np.load関数を使って、対応するキーのデータをパラメータに読み込む
...
#重みの生成をforwardメソッドの中で行うことでｍLinearクラスの入力サイズを自動で決められる。
    def forward(self, x):
        if self.W.data is None:
            self.in_size = x.shape[1]
            self._init_W()
...
#__dict__ にはすべてのインスタンス変数がディクショナリ型で格納されているので、parameter インスタンスだけを取り出すことができる。
...
class RNN(Layer):
    #ここでは、hidden_sizeとin_sizeだけを受け取る。
    #入力サイズは、順伝播するときに自動で求められる。
    def __init__(self, hidden_size, in_size=None):
        ...
        """
        super().__init__()
        #x2h：入力xから隠れ状態hへと変換する全結合層
        self.x2h = Linear(hidden_size, in_size=in_size)
        #h2h：前の隠れ状態から次の隠れ状態へと変換する全結合層
        self.h2h = Linear(hidden_size, in_size=in_size, nobias=True)
        self.h = None
    
    #self.hの有無によって処理を切り替える。reset_stateは隠れ状態をリセットするための物
