...
def linear_simple(x, W, b=None):
    ...
    y = t + b
    #ここでtのデータを消去している！メモリの観点からいらないものを消している。
    t.data = None
    return y
...
def softmax_cross_entropy_simple(x, t):
    ...
    p = softmax_simple(x)
    #log(0)を防ぎたいから、pの値を1e-15とする
    #clip関数がcuda使っていて実装できない。（泣）
    #p = clip(p, 1e-15, 1.0)
    log_p = log(p)
...
#yとtの正解率を算出する。
#y：NNの予測結果、ｔ：正解データ
#predとして予測結果を求める→予測結果の最大のインデックスを求めてリシェイプする。→predと正解データのtを比較
#True / Falseのテンソルになる
#へいきんをもとめると、その値が正解率になる
def accuracy(y, t):
...
#ReLUはx=0 or x<0 なら０を出力、それ以外はそのまま
#maskは出力側から伝わる勾配を通すか通さないか用のもの
class ReLU(Function):
    def forward(self, x):
        y = np.maximum(x, 0.0)
        return y
...
#dropoutはFunctionクラスを継承して、クラスを作ってもよいが、dropoutは処理が簡単なので、省く
